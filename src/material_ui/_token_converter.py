...
def group_tokens_by_component(tokens: ParsedTokens) -> dict[str, list[str]]:
    ...
    for token_name in tokens.keys():
        match = re.search(r"^(md\.comp\..+?)\.", token_name)
        if match:
            group_name = match.group(1)
            ret_val[group_name].append(token_name)
    return ret_val
...
def generate_component_py_files(tokens):
    component_groups = group_tokens_by_component(tokens)
    for group_name, token_names in component_groups.items():
        with open(TOKENS_OUT_PATH / f"{to_python_name(group_name)}.py", "w") as f:
            f.write(
                ...
                f"# Instead, run the script to regenerate it.\n"
            )
            for token_name in token_names:
                # Strip the group name as the file name already has the 'group'.
                var_name = to_python_name(token_name[len(group_name) + 1 :])
                token_value = tokens[token_name]
                f.write(f"{var_name} = {token_value!r}\n")
